...
import { Smartphone, AlertTriangle, PartyPopper } from "lucide-react";
...
export default function FriendPage() {
  ...
  // Fetch all messages + subscribe to new ones
  useEffect(() => {
    // Initial fetch
    (async () => {
        ...
        <div className="mx-2 overflow-hidden rounded-[2rem] border-4 border-black bg-background shadow-2xl">
          ...
          <div className="flex items-center gap-3 border-b bg-primary/5 px-5 py-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">SOBR Safety Alerts</p>
            </div>
          </div>
          ...
          <div className="min-h-[70vh] flex flex-col">
            {/* Listening banner */}
            <div className="mx-4 mt-4 flex items-center gap-2 rounded-lg bg-emerald-500/10 px-4 py-2">
              <span className="relative flex size-2">
                <span className="absolute inline-flex size-full animate-ping rounded-full bg-emerald-400 opacity-75" />
                <span className="relative inline-flex size-2 rounded-full bg-emerald-500" />
              </span>
              <span className="text-xs font-medium text-emerald-700 dark:text-emerald-400">
                Listening for safety alerts…
              </span>
            </div>
                        ...
                        <div className="flex-1 space-y-1">
                          <div className="flex items-center justify-between">
                            <span className={`text-xs font-semibold uppercase tracking-wider ${
                              ...
                              {msg.type === "high-risk" ? "⚠️ Safety Alert" : "🍻 Session Ended"}
                            </span>
